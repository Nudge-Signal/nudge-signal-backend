...
- 모든 DB/Redis 접속 정보는 환경변수로 주입 (`DB_HOST`, `DB_PORT`, `DB_NAME`, `DB_USERNAME`, `DB_PASSWORD`, `REDIS_HOST`, `REDIS_PORT`, `REDIS_PASSWORD`).
- 로컬 개발 시에는 `.env.example` 을 `.env` 로 복사해 사용. `.env` 는 절대 커밋하지 않는다.
- 새 설정값을 추가할 때는 반드시 `application-{profile}.yml` 에도 동일 키를 넣고 `.env.example` 을 업데이트한다.
