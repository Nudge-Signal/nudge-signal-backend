...
1. **main 동기화 체크** (2인 팀 필수)
   ...
   ```
   결과가 비어 있지 않으면:
   - 새 작업: `git switch main` 후 `git pull --ff-only`, 그리고 새 브랜치 분기
   - 진행 중 브랜치: `git rebase origin/main`
2. **Plan Mode 진입**: 어떠한 작업이든 시작 전 반드시 Codex Plan Mode 로 계획을 세운다. 단순 문서 수정, 한 줄 변경, 질문성 조사도 예외 없이 Plan Mode 를 먼저 사용한다.
3. feature 브랜치에서 구현 시작 (`feat/<scope>`, `fix/<scope>`, `chore/<scope>` 등).
...
1. 영향받는 문서를 같은 변경 안에 갱신 (`README.md` / `AGENTS.md` / `.codex/*` / `docs/TROUBLESHOOTING.md` 등).
...
자동 기록 도우미: `.codex/agents/troubleshooting-recorder.toml` (Task 로 호출). 보수적 판단: 단순 코드 작성·정상 빌드는 기록 대상 아님.
...
- `create-pr` — PR 생성 보조.
- `troubleshooting-recorder` — 세션 종료 시 트러블슈팅 자동 기록.
- `architecture-alignment` — 신규 코드 / 리팩토링이 레이어 / DDD 원칙에 맞는지 점검.
