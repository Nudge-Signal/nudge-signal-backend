...
이 파일은 이 저장소에서 작업하는 Codex (또는 다른 AI 코딩 도우미) 가 따라야 할 가이드입니다. 행동 규칙과 절차는 분리해서 관리합니다 — 이 파일은 **프로젝트 사실** (스택 / 구조 / 명령) 만 둡니다.

- @.codex/Rule.md — 반드시 따라야 하는 규칙 (워크플로 / Git / 보안 / 컨벤션 / 코드 품질)
- @.codex/Skill.md — 자주 쓰는 절차·루틴 (작업 시작 / 검증 / 마무리 / 트러블슈팅 기록 / 환경변수 추가)
- @.codex/Review.md — 코드 리뷰 기준과 findings 출력 형식
...
- 그 외 자세한 키 / 보안 / fallback 금지 규칙은 `.codex/Rule.md` 의 **보안** 섹션 참조.
